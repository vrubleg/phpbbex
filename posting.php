...
if ($mode == 'edit' && !$auth->acl_get('m_edit', $forum_id))
{
	...
	if (!($post_data['post_time'] > time() - ($config['edit_time'] * 60) || !$config['edit_time'] || $auth->acl_get('u_ignoreedittime')
		|| ($post_data['topic_first_post_id'] == $post_id && $auth->acl_get('u_ignorefpedittime'))))
	{
...
$uninit = ['post_attachment' => 0, 'poster_id' => $user->data['user_id'], 'enable_magic_url' => 0, 'topic_status' => 0, 'topic_type' => POST_NORMAL, 'topic_priority' => 0, 'post_subject' => '', 'topic_title' => '', 'post_time' => 0, 'post_edit_reason' => '', 'notify_set' => 0];
...
if ($mode != 'edit')
{
	$post_data['enable_sig']        = ($config['allow_sig'] && $user->optionget('attachsig'));
	$post_data['enable_smilies']    = ($config['allow_smilies'] && $user->optionget('smilies'));
	$post_data['enable_bbcode']     = ($config['allow_bbcode'] && $user->optionget('bbcode'));
...
if ($submit || $preview || $refresh)
{
	...
	if ($config['enable_post_confirm'] && !$user->data['is_registered'] && in_array($mode, ['quote', 'post', 'reply']))
	{
		$captcha_data = [
			'message'   => utf8_normalize_nfc(request_var('message', '', true)),
			'subject'   => utf8_normalize_nfc(request_var('subject', '', true)),
...
if (!sizeof($error) && $preview)
{
	...
	&& $auth->acl_get('f_poll', $forum_id))
	{
		...
		$template->assign_vars([
			...
			'S_IS_MULTI_CHOICE'     => ($post_data['poll_max_options'] > 1),
...
$s_do_merge_checked = $s_do_merge_allowed && (($submit || $preview || $refresh) ? request_var('do_merge', false) : (($post_data['topic_first_post_id'] != $post_data['topic_last_post_id']) && ($current_time - $post_data['topic_last_post_time'] < intval($config['merge_interval']) * 3600)));
	...
	&& $auth->acl_get('f_poll', $forum_id))
{
	$template->assign_vars([
		...
		'S_POLL_RESET_CHECKED'  => !empty($poll_reset),
