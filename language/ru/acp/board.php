...
$lang = array_merge($lang, array(
	...
	'AVATAR_GALLERY_PATH_EXPLAIN'	=> 'Путь к папке с загруженными в неё рисунками относительно корневой папки phpBBex. Например, <samp>images/avatars/gallery</samp>.',
	'AVATAR_STORAGE_PATH'			=> 'Папка для загрузки аватар',
	'AVATAR_STORAGE_PATH_EXPLAIN'	=> 'Путь к папке относительно корневой папки phpBBex. Например, <samp>images/avatars/upload</samp>.',
...
$lang = array_merge($lang, array(
	...
	'LIMIT_LOAD_EXPLAIN'			=> 'Если средняя ежеминутная нагрузка на сервер превышает заданное значение, конференция будет автоматически отключена. Значение, равное 1.0, означает ≈100% использование ресурсов одного процессора. Эта возможность применима только для серверов на основе UNIX. Значение будет сброшено в 0, если phpBBex не сможет получить предел загрузки системы.',
...
// Auth settings
$lang = array_merge($lang, array(
	'ACP_AUTH_SETTINGS_EXPLAIN'	=> 'phpBBex поддерживает расширения аутентификации, или модули. Они позволяют вам установить способ аутентификации пользователей при их входе на конференцию. По умолчанию доступны три модуля: DB, LDAP и Apache. Не все методы требуют дополнительной информации, поэтому заполняйте только те поля, которые необходимы для выбранного метода.',
...
$lang = array_merge($lang, array(
	...
	'ICONS_PATH_EXPLAIN'		=> 'Путь относительно корневой папки phpBBex. Например, <samp>images/icons</samp>.',
	'PATH_SETTINGS'				=> 'Параметры путей',
	'RANKS_PATH'				=> 'Путь к картинкам званий',
	'RANKS_PATH_EXPLAIN'		=> 'Путь относительно корневой папки phpBBex. Например, <samp>images/ranks</samp>.',
	'SCRIPT_PATH'				=> 'Путь к конференции',
	'SCRIPT_PATH_EXPLAIN'		=> 'Путь к папке, содержащей phpBBex, относительно имени домена. Например, <samp>/phpBBex</samp>.',
	...
	'SMILIES_PATH_EXPLAIN'		=> 'Путь относительно корневой папки phpBBex. Например, <samp>images/smilies</samp>.',
	'UPLOAD_ICONS_PATH'			=> 'Путь к значкам групп расширений',
	'UPLOAD_ICONS_PATH_EXPLAIN'	=> 'Путь относительно корневой папки phpBBex. Например, <samp>images/upload_icons</samp>.',
));
