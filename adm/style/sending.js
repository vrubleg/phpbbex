jQuery(function($)
{
	var submit_selector = 'input[type=submit], button[type=submit]';
	...
	$('form').find(submit_selector).on('click', function()
	{
		var $submit = $(this);
	...
	$('form').find(submit_selector).filter('.default-submit-action').attr('title', 'Ctrl+Enter');
	$('form').find(submit_selector).filter('.alternate-submit-action').attr('title', 'Alt+Enter');
	...
	$('form input[type=text], form input[type=password], form textarea').on('keydown', function (e)
	{
		...
		if ((e.which == 13 || e.which == 10) && (is_input || e.ctrlKey || e.altKey))
		{
			...
			var $submit = $form.find(submit_selector).filter('.' + (e.altKey ? 'alternate' : 'default') + '-submit-action').eq(0);
			if ($submit.length == 0)
			{
				if (e.altKey) return false;
				$submit = $form.find(submit_selector);
				if ($submit.length == 0) return false;
				if ($submit.length > 1)
				{
					$submit = $submit.filter('[name=submit]');
