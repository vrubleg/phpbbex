	...
	function check_server_requirements($mode, $sub)
	{
		...
		if (version_compare($gallery_version, '0.0.0', '>'))
		{
			$template->assign_block_vars('checks', [
				...
				'LEGEND_EXPLAIN'    => sprintf($user->lang['FOUND_INSTALL_EXPLAIN'], '<a href="' . append_sid($this->p_master->module_url, 'mode=update') . '">', '</a>'),
			]);
			$template->assign_block_vars('checks', [
		...
		$url = (!in_array(false, $passed)) ? append_sid($this->p_master->module_url, "mode={$mode}&amp;sub=create_table") : append_sid($this->p_master->module_url, "mode={$mode}&amp;sub=requirements");
