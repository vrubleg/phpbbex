...
phpbb_gallery::setup(['viewtopic', 'mods/gallery', 'mods/exif_data']);
...
if (!phpbb_gallery::$auth->acl_check('i_view', $album_id, $album_data['album_user_id']) || ($image_data['image_status'] == phpbb_gallery_image::STATUS_ORPHAN))
{
	...
	{
		login_box(phpbb_gallery_url::append_sid('relative', 'image_page', "image_id={$image_id}"), $user->lang['LOGIN_EXPLAIN_GALLERY_VIEW']);
	}
	else
	{
...
if (in_array($mode, ['watch', 'unwatch', 'favorite', 'unfavorite']) && check_link_hash($token, "{$mode}_{$image_id}"))
{
	$backlink = phpbb_gallery_url::append_sid('image_page', "image_id={$image_id}");
...
$sort_by_sql = ['t' => 'image_time', 'n' => 'image_name_clean', 'u' => 'image_username_clean', 'ra' => 'image_rate_avg', 'r' => 'image_rates', 'c' => 'image_comments', 'lc' => 'image_last_comment'];
...
$template->assign_vars([
	...
	'U_DELETE'          => ($s_allowed_delete) ? phpbb_gallery_url::append_sid('posting', "mode=delete&amp;image_id={$image_id}") : '',
	'U_EDIT'            => ($s_allowed_edit) ? phpbb_gallery_url::append_sid('posting', "mode=edit&amp;image_id={$image_id}") : '',
	'U_REPORT'          => (phpbb_gallery::$auth->acl_check('i_report', $album_id, $album_data['album_user_id']) && ($image_data['image_user_id'] != $user->data['user_id'])) ? phpbb_gallery_url::append_sid('posting', "mode=report&amp;image_id={$image_id}") : '',
	...
	'S_ALBUM_ACTION'    => phpbb_gallery_url::append_sid('image_page', "image_id={$image_id}"),
...
if (phpbb_gallery_config::get('allow_rates'))
{
	...
	$template->assign_vars([
		...
		'S_COMMENT_ACTION'      => phpbb_gallery_url::append_sid('comment', "image_id={$image_id}&amp;mode=rate"),
	]);
	unset($rating);
}
...
if (!$comments_disabled && phpbb_gallery::$auth->acl_check('c_post', $album_id, $album_data['album_user_id']) && ($album_data['album_status'] != ITEM_LOCKED) && (($image_data['image_status'] != phpbb_gallery_image::STATUS_LOCKED) || phpbb_gallery::$auth->acl_check('m_status', $album_id, $album_data['album_user_id'])))
{
	...
	$template->assign_var('S_COMMENT_ACTION', phpbb_gallery_url::append_sid('comment', "image_id={$image_id}&amp;mode=add"));
}
	...
	if ($image_data['image_comments'] > 0)
	{
		...
		foreach ($comments as $row)
		{
			...
			$template->assign_block_vars('commentrow', [
				'U_COMMENT'     => phpbb_gallery_url::append_sid('image_page', "image_id={$image_id}&amp;start={$start}&amp;sort_order={$sort_order}") . '#comment_' . $row['comment_id'],
				...
				'POSTER_WITH_US'    => $user_cache[$user_id]['with_us'],
				'POSTER_POSTS'      => $user_cache[$user_id]['posts'],
				'POSTER_TOPICS'     => $user_cache[$user_id]['topics'],
				...
				'S_POSTER_GENDER_X' => $user_cache[$user_id]['gender'] == GENDER_X,
				'S_POSTER_GENDER_M' => $user_cache[$user_id]['gender'] == GENDER_M,
				'S_POSTER_GENDER_F' => $user_cache[$user_id]['gender'] == GENDER_F,

				'S_POSTER_RATING'           => $config['rate_enabled'] && (!$config['rate_no_negative'] || !$config['rate_no_positive']) && ($user_id != ANONYMOUS),
				'POSTER_RATING'             => $user_cache[$user_id]['rating'],
				'POSTER_RATING_POSITIVE'    => $user_cache[$user_id]['rating_positive'],
				'POSTER_RATING_NEGATIVE'    => $user_cache[$user_id]['rating_negative'],
				'POSTER_RATED'              => $user_cache[$user_id]['rated'],
		...
		$template->assign_vars([
			...
			'PAGINATION'        => generate_pagination(phpbb_gallery_url::append_sid('image_page', "image_id={$image_id}&amp;sort_order={$sort_order}"), $image_data['image_comments'], $config['posts_per_page'], $start),
		]);
	}
}
...
if (!isset($user_cache[$image_data['image_user_id']]))
{
	$sql = $db->sql_build_query('SELECT', [
		...
		'WHERE'     => $db->sql_in_set('u.user_id', [ANONYMOUS, $image_data['image_user_id']]),
	]);
	$result = $db->sql_query($sql);
