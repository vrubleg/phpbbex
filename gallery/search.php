...
phpbb_gallery::setup(['mods/gallery', 'search']);
...
$search_album   = request_var('aid', [0]);
...
$sort_by_text = ['t' => $user->lang['TIME'], 'n' => $user->lang['IMAGE_NAME'], 'u' => $user->lang['SORT_USERNAME']];
$sort_by_sql = ['t' => 'image_time', 'n' => 'image_name_clean', 'u' => 'image_username_clean'];
...
if ($keywords || $username || $user_id || $search_id || $submit)
{
	...
	if ($username)
	{
		if ((strpos($username, '*') !== false) && (utf8_strlen(str_replace(['*', '%'], '', $username)) < $config['min_search_author_chars']))
		{
	...
	$hilit = explode(' ', preg_replace('#\s+#u', ' ', str_replace(['+', '-', '|', '(', ')', '&quot;'], ' ', $keywords)));
		...
		else
		{
			...
			while ($commentrow = $db->sql_fetchrow($result))
			{
				...
				$template->assign_block_vars('commentrow', [
					'U_COMMENT'     => phpbb_gallery_url::append_sid('image_page', "image_id={$image_id}") . '#' . $commentrow['comment_id'],
					...
					'U_DELETE'      => (phpbb_gallery::$auth->acl_check('m_comments', $album_id) || (phpbb_gallery::$auth->acl_check('c_delete', $album_id) && ($commentrow['comment_user_id'] == $user->data['user_id']) && $user->data['is_registered'])) ? phpbb_gallery_url::append_sid('comment', "image_id={$image_id}&amp;mode=delete&amp;comment_id=" . $commentrow['comment_id']) : '',
					'U_QUOTE'       => (phpbb_gallery::$auth->acl_check('c_post', $album_id)) ? phpbb_gallery_url::append_sid('comment', "image_id={$image_id}&amp;mode=add&amp;comment_id=" . $commentrow['comment_id']) : '',
					'U_EDIT'        => (phpbb_gallery::$auth->acl_check('m_comments', $album_id) || (phpbb_gallery::$auth->acl_check('c_edit', $album_id) && ($commentrow['comment_user_id'] == $user->data['user_id']) && $user->data['is_registered'])) ? phpbb_gallery_url::append_sid('comment', "image_id={$image_id}&amp;mode=edit&amp;comment_id=" . $commentrow['comment_id']) : '',
