...
function view_message($id, $mode, $folder_id, $msg_id, $folder, $message_row)
{
	...
	if ($message_row['message_edit_count'] && $config['display_last_edited'])
	{
		...
		$l_edited_by = sprintf($l_edit_time_total, $message_row['username'], $user->format_date($message_row['message_edit_time'], false, true), $message_row['message_edit_count']);
	}
	else
	{
		$l_edited_by = '';
	...
	$template->assign_vars([
		...
		'ONLINE_IMG'        => (!$config['load_onlinetrack']) ? '' : ((isset($user_info['online']) && $user_info['online']) ? $user->img('icon_user_online', 'ONLINE') : $user->img('icon_user_offline', 'OFFLINE')),
		'S_ONLINE'          => (!$config['load_onlinetrack']) ? false : (isset($user_info['online']) && $user_info['online']),
		'DELETE_IMG'        => $user->img('icon_post_delete', 'DELETE_MESSAGE'),
		'INFO_IMG'          => $user->img('icon_post_info', 'VIEW_PM_INFO'),
		'PROFILE_IMG'       => $user->img('icon_user_profile', 'READ_PROFILE'),
		'EMAIL_IMG'         => $user->img('icon_contact_email', 'SEND_EMAIL'),
		'QUOTE_IMG'         => $user->img('icon_post_quote', 'POST_QUOTE_PM'),
		'REPLY_IMG'         => $user->img('button_pm_reply', 'POST_REPLY_PM'),
		'REPORT_IMG'        => $user->img('icon_post_report', 'REPORT_PM'),
		'EDIT_IMG'          => $user->img('icon_post_edit', 'POST_EDIT_PM'),
