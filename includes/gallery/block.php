...
class phpbb_gallery_block
{
	...
	*/
	public function set_modes($modes = [])
	{
		...
		$allowed_modes = ['recent', 'random', 'comment'];
	...
	*/
	public function set_display_options($options = [])
	{
		...
		$allowed_options = ['albumname', 'comments', 'imagename', 'imagetime', 'imageviews', 'username', 'ratings', 'ip'];
	...
	*/
	private $auth_moderate = [];
	private $auth_view = [];
	private $auth_comments = [];
	...
	private function display_comments()
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$template->assign_block_vars($this->template_block_comments, [
				'U_COMMENT'     => phpbb_gallery_url::append_sid('image_page', "image_id={$image_id}") . '#comment_' . $row['comment_id'],
				...
				'U_DELETE'      => (phpbb_gallery::$auth->acl_check('m_comments', $album_id) || (phpbb_gallery::$auth->acl_check('c_delete', $album_id) && ($row['comment_user_id'] == $user->data['user_id']) && $user->data['is_registered'])) ? phpbb_gallery_url::append_sid('comment', "image_id={$image_id}&amp;mode=delete&amp;comment_id=" . $row['comment_id']) : '',
				'U_QUOTE'       => (phpbb_gallery::$auth->acl_check('c_post', $album_id)) ? phpbb_gallery_url::append_sid('comment', "image_id={$image_id}&amp;mode=add&amp;comment_id=" . $row['comment_id']) : '',
				'U_EDIT'        => (phpbb_gallery::$auth->acl_check('m_comments', $album_id) || (phpbb_gallery::$auth->acl_check('c_edit', $album_id) && ($row['comment_user_id'] == $user->data['user_id']) && $user->data['is_registered'])) ? phpbb_gallery_url::append_sid('comment', "image_id={$image_id}&amp;mode=edit&amp;comment_id=" . $row['comment_id']) : '',
