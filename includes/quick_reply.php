...
$s_quick_reply_userprefs = (bool) $user->optionget('viewquickreply');
$s_quick_reply_guests = ($user->data['user_id'] == ANONYMOUS && $config['allow_quick_reply'] == 2);
$s_quick_reply_display = ($user->data['user_id'] == ANONYMOUS) ? $s_quick_reply_guests : $s_quick_reply_userprefs;

if ($config['allow_quick_reply'] && $s_quick_reply_display)
{
	...
	if ($topic_data['forum_type'] != FORUM_POST)
	{
		$s_quick_reply = false;
	}
...
{
	if (!isset($topic_data[$var_name]))
	{
		$topic_data[$var_name] = $default_value;
	}
}
unset($uninit);
...
if ($config['allow_quick_reply_smilies'])
{
	generate_smilies();
}
...
$lock_topic_checked = (isset($topic_data['topic_status']) && $topic_data['topic_status'] == ITEM_LOCKED) ? 1 : 0;
...
if ($config['allow_topic_notify'] && $user->data['is_registered'])
{
	...
	$topic_data['notify_set'] = (int) $db->sql_fetchfield('topic_id');
	$db->sql_freeresult($result);
}

// If the user is replying and not already watching this topic but set to always being notified we need to overwrite this setting
$notify_set         = ($config['allow_topic_notify'] && $user->data['is_registered'] && !$topic_data['notify_set']) ? $user->data['user_notify'] : $topic_data['notify_set'];
$notify_checked     = $notify_set;

$s_action = append_sid(PHPBB_ROOT_PATH . 'posting.php', "mode=reply&amp;t={$topic_id}");
...
$qr_hidden_fields = [
	'topic_cur_post_id' => (isset($topic_data['topic_last_post_id'])) ? (int) $topic_data['topic_last_post_id'] : 0,
];

// Attachment entry
$show_attach_box = (PHP_FILE_UPLOADS && $auth->acl_get('f_attach', $forum_id) && $auth->acl_get('u_attach') && $config['allow_attachments'] && $config['allow_quick_reply_attachbox']);
...
$template->assign_vars([
	...
	'SUBJECT'               => '',
	'EXTRA_OPTIONS_DISPLAY' => ($config['allow_quick_reply_checkboxes']),
	...
	'L_QUICK_REPLY'             => $user->lang['QUICK_REPLY'],
	'L_ICON'                    => $user->lang['POST_ICON'],
	'L_MESSAGE_BODY_EXPLAIN'    => (intval($config['max_post_chars'])) ? sprintf($user->lang['MESSAGE_BODY_EXPLAIN'], intval($config['max_post_chars'])) : '',

	'S_DISPLAY_USERNAME'        => (!$user->data['is_registered']),
	'S_SHOW_TOPIC_ICONS'        => false,
	'S_SUBJECT_ALLOWED'         => $config['allow_quick_reply_subject'],
	'S_BBCODE_ALLOWED'          => $bbcode_status,
	'S_BBCODE_CHECKED'          => ($bbcode_checked) ? ' checked="checked"' : '',
	'S_SMILIES_ALLOWED'         => ($smilies_status && $config['allow_quick_reply_smilies']),
	...
	'S_LOCK_TOPIC_ALLOWED'      => ($auth->acl_get('m_lock', $forum_id) || ($auth->acl_get('f_user_lock', $forum_id) && $user->data['is_registered'] && !empty($topic_data['topic_poster']) && $user->data['user_id'] == $topic_data['topic_poster'] && $topic_data['topic_status'] == ITEM_UNLOCKED)),
	'S_LOCK_TOPIC_CHECKED'      => ($lock_topic_checked) ? ' checked="checked"' : '',
	'S_LINKS_ALLOWED'           => $url_status,
	'S_MAGIC_URL_CHECKED'       => ($urls_checked) ? ' checked="checked"' : '',
	'S_SAVE_ALLOWED'            => $user->data['is_registered'],
	'S_HAS_DRAFTS'              => $has_drafts,
	'S_FIRST_POST_SHOW_ALLOWED' => false,
	'S_NEW_MESSAGE'             => false,
