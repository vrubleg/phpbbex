...
function mcp_front_view($id, $mode, $action)
{
	...
	if ($module->loaded('pm_reports') && $auth->acl_getf_global('m_report'))
	{
		...
		if ($total)
		{
			...
			$sql = $db->sql_build_query('SELECT', [
				'SELECT'    => 'r.report_id, r.report_time, p.msg_id, p.message_subject, p.message_time, p.to_address, u.username, u.username_clean, u.user_colour, u.user_id, u2.username as author_name, u2.username_clean as author_name_clean, u2.user_colour as author_colour, u2.user_id as author_id',
