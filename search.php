...
// Define some vars
$limit_days     = [0 => $user->lang['ALL_RESULTS'], 7 => $user->lang['7_DAYS'], 30 => $user->lang['1_MONTH'], 90 => $user->lang['3_MONTHS'], 180 => $user->lang['6_MONTHS'], 365 => $user->lang['1_YEAR']];
$sort_by_text   = ['r' => $user->lang['SORT_RELEVANCE'], 't' => $user->lang['SORT_TIME'], 'c' => $user->lang['CREATION_TIME']];
...
if ($keywords || $author || $author_id || $search_id || $submit)
{
	...
	{
		if ((strpos($author, '*') !== false) && (utf8_strlen(str_replace(['*', '%'], '', $author)) < $config['min_search_author_chars']))
		{
	...
	$sort_by_sql = ['r' => 'relevance', 't' => (($show_results == 'posts') ? 'p.post_time' : 't.topic_last_post_time'), 'c' => (($show_results == 'posts') ? 'p.post_time' : 't.topic_time')];
	...
	{
		$firstpost_only = ($search_fields === 'firstpost' || $search_fields == 'titleonly');
	...
	if (sizeof($id_ary))
	{
		...
		$sql_where .= ($show_results == 'posts') ? $m_approve_fid_sql : str_replace(['p.post_approved', 'p.forum_id'], ['t.topic_approved', 't.forum_id'], $m_approve_fid_sql);
	}
	...
	$template->assign_vars([
		...
		'S_SHOW_TOPICS'         => ($show_results != 'posts'),
	...
	if ($sql_where)
	{
		...
		foreach ($rowset as $row)
		{
			...
			$template->assign_block_vars('searchresults', array_merge($tpl_ary, [
				'FORUM_ID'          => $forum_id,
			...
			if ($config['rate_enabled'] && $config['display_raters'] && $show_results == 'posts' && isset($post_raters[$row['post_id']]))
			{
				...
				{
					$template->assign_block_vars('searchresults.postrater', [
...
$template->assign_vars([
	'S_SEARCH_ACTION'       => append_sid(PHPBB_ROOT_PATH . 'search.php'),
